...
class User extends MY_Controller {
	...
	/**
	 * This function is used to load login page view if the user is not 
	 * Logged in
	 * @return Void
	 */
	public function login() {
		if (isset($this->session->get_userdata()['user_details'])) {
			redirect($this->base_url . 'user/profile', 'refresh');
		}
	 ...
	 * @return Void
	 */
	public function authentication($page = '') {
	...
	public function register() {
		...
		$chk_email = $this->User_model->check_exists(TAB_USERS, 'email', $post->email);
		if( $chk_email ) {
			$arr = array(
			...
			$this->model->insert(TAB_USERS, $arr);
			$res = 'done';
		} else {
			$res = 'email_exist';
		}
...
/*	public function dataTable() {
	...
	public function updateProfile() {
		...
		if($post['type'] == 'general') {
			...
			$this->model->update(TAB_USERS, $upd, array('id' => $post['userId']));
			$art_msg['msg'] 	= 'User detail successfully updated';
			$art_msg['type'] 	= 'success';
		}

		if($post['type'] == 'password') {
			$res = $this->model->getData(TAB_USERS, array('id' => $post['userId']), '');
			if (password_verify($post['current_password'], $res[0]->password)) {
				$upd = array(
					"password" => password_hash($post['new_password'], PASSWORD_DEFAULT)
				);
				$this->model->update(TAB_USERS, $upd, array('id' => $post['userId']));
				$art_msg['msg'] 	= 'Password successfully updated';
				$art_msg['type'] 	= 'success';
			} else {
				$art_msg['msg']  = 'Entered wrong current password!';
				$art_msg['type'] = 'error';
			}
		...
		$this->session->set_userdata('alert_msg', $art_msg);
		//die;
		redirect(baseurl() . 'user/profile', 'refresh');
	}
	...
	public function uploadProfile() {
		...
		$extention = explode('/', $img[0]);
		$extention = $extention[1];
		$path = realpath(dirname(dirname(dirname(dirname(dirname(__FILE__)))))) . '/assets/images/profile_' . $post['user'] . '.' . $extention;
		$data = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $post['img']));

		file_put_contents($path, $data);
		$upt = array(
			"profile_pic" => basename($path)
		);
		$this->model->update(TAB_USERS, $upt, array("id" => $post['user']));
	}
	...
	public function checEmailExist() {
		$result = 1;
		$res = $this->User_model->get_data('users', $this->input->post('email'), 'email');
		if (!empty($res)) {
